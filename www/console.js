...
document.addEventListener('deviceready', function () {
  // copy the PhoneGap features in to the sandbox window by working out the difference between the
  // navigator objects and copying across those that are missing
  for (var key in window.navigator) {
    if (typeof sandboxframe.contentWindow.navigator[key] == 'undefined') {
      sandboxframe.contentWindow.navigator[key] = window.navigator[key];
    }
  }
  
  // websockets too :)
  sandboxframe.contentWindow.WebSocket = window.WebSocket;
